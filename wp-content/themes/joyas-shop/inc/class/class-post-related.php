...
class joyas_shop_Post_Related {
	...
	public function __construct() {
		...
		if( !is_admin()  )
		{
			add_action( 'joyas_shop_site_content_type', array( $this,'site_loop_heading' ), 10 ); 
			add_action( 'joyas_shop_site_content_type', array( $this,'site_content_type' ), 30 ); 
		}
		
		add_action( 'joyas_shop_posts_blog_media', array( $this, 'render_thumbnail' ) ); 
		
		
		add_action( 'joyas_shop_loop_navigation', array( $this,'site_loop_navigation' ) );
		add_action( 'joyas_shop_single_post_navigation', array( $this,'single_post_navigation' ),10 ); 
		...
		add_action( 'joyas_shop_single_post_navigation', array( $this,'author_bio' ),20 );
	}
    ...
    function site_content_type( ){
		...
		$type = apply_filters( 'joyas_shop_content_type_filter', joyas_shop_get_option( 'blog_loop_content_type' ) );
	...
	*/
	public function content_read_more_link( $more  ) {
		...
		return sprintf( '<div class="more-link">
		    ...
		    esc_html( joyas_shop_get_option( 'read_more_text' ) )
        );
	...
	*/
	...
	function single_post_navigation( ) {
		...
		if( $prevPost ) :
			$html .= '<div class="col-md-6 col-sm-6"><div class="row align-items-center">';
				...
				$html .='<div class="col-md-9"><h5>'.esc_html__('Previous Article','joyas-shop').'</h5>';
		...
		if( $nextPost ) : 
			$html .= '<div class="col-md-6 col-sm-6"><div class="row align-items-center">';
				...
				$html .='<div class="col-md-9 ml-auto align-right"><h5>'.esc_html__('Next Article','joyas-shop').'</h5>';
		...
		$html  	 = apply_filters( 'joyas_shop_single_post_navigation_filter', $html );
	...
	function site_loop_navigation( $type = '' ) {
		...
		if( $type == '' ){
			$type = apply_filters( 'joyas_shop_loop_navigation_filter', get_theme_mod( 'joyas_shop_loop_navigation', 'default' ) );
		}
		...
		if( $type == 'default' ):
			...
			the_posts_navigation(
				array(
					'prev_text' => '<span class="btn-wrap">'.esc_html__('Previous Posts', 'joyas-shop').'</span><span class="icon"><i class="icofont-rounded-double-left"></i></span>',
					'next_text' => '<span class="btn-wrap">'.esc_html__('Next Posts', 'joyas-shop').'</span><span class="icon"><i class="icofont-rounded-double-right"></i></span>',
					'screen_reader_text' => __('Posts navigation', 'joyas-shop')
				)
			);
			...
			the_posts_pagination( array(
				...
				'type' => 'list',
				'mid_size' => 2,
				'prev_text' => esc_html__( 'Previous', 'joyas-shop' ),
				'next_text' => esc_html__( 'Next', 'joyas-shop' ),
				'screen_reader_text' => esc_html__( '&nbsp;', 'joyas-shop' ),
			) );
		echo '</div>';
		endif;
	...
	public function get_gallery_thumbnail(){
		...
		if( has_block('gallery', $post->post_content) ): 
			...
			if( !empty( $post_blocks ) ):
				$html .= '<i class="icofont-ui-image joyas-shop-icon"></i>';
		...
		$html =  apply_filters( 'joyas-shop_gallery_thumbnail', $html );
	...
	public function get_audio_thumbnail(){
		...
		{	 $i = 0;
			...
			$html  = '<div class="img-box">';
			$html .='<i class="icofont-audio joyas-shop-icon"></i>';
		...
		$html =  apply_filters( 'joyas-shop_audio_thumbnail', $html );
	...
	public function get_video_thumbnail(){
		...
		{	
			$html = '<div class="img-box">';
			$i = 0;
			$html .='<i class="icofont-video joyas-shop-icon"></i>';
		...
		$html =  apply_filters( 'joyas-shop_video_thumbnail', $html );
	...
	public function get_image_thumbnail(){
		...
		$html =  apply_filters( 'joyas-shop_image_thumbnail', $html );
...
$joyas_shop_post_related = new joyas_shop_Post_Related();